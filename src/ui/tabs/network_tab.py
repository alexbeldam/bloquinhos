...
class NetworkTab(SettingsTab):
    ...
    POLICY_LABELS = {
        "auto": "network_settings.policies.auto.label",
        "manual": "network_settings.policies.manual.label",
        "always": "network_settings.policies.always.label",
    }

    POLICY_DESCRIPTIONS = {
        "auto": "network_settings.policies.auto.description",
        "manual": "network_settings.policies.manual.description",
        "always": "network_settings.policies.always.description",
    }
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            order=30,
            category="network",
            title_key="settings.tabs.network",
        )
    ...
    def render(
        ...
        current_y = self._render_checkbox_option(
            surface,
            rect,
            tr("network_settings.start_offline"),
            "network.start_offline",
            start_offline,
            current_y,
        )
        
        help_text = tr("network_settings.start_offline_help")
        ...
        current_y = self._render_dropdown_option(
            surface,
            rect,
            tr("network_settings.reconnection_policy"),
            reconnect_policy,
            current_y,
        )
        ...
        description_key = self.POLICY_DESCRIPTIONS.get(display_option, "")
        description = tr(description_key) if description_key else ""
        if description:
    ...
    def _render_dropdown_option(
        ...
        value_key = self.POLICY_LABELS.get(current_value)
        value_text = tr(value_key) if value_key else current_value.capitalize()
        ...
        if self._dropdown_open:
            ...
            for idx, option in enumerate(self._options_list):
                ...
                option_key = self.POLICY_LABELS.get(option)
                option_label = tr(option_key) if option_key else option.capitalize()
